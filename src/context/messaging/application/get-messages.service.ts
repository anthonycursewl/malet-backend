...
export class GetMessagesService implements GetMessagesUseCase {
    ...
    async execute(userId: string, params: GetMessagesParams): Promise<GetMessagesResult> {
        ...
        // Construir resultado
        const messagesWithSender: MessageWithSender[] = resultMessages.map(message => {
        ...
        return {
            ...
            hasMore,
            oldestMessageDate: resultMessages.length > 0
                ? resultMessages[resultMessages.length - 1].getCreatedAt()
                : null,
            newestMessageDate: resultMessages.length > 0
                ? resultMessages[0].getCreatedAt()
                : null
        };
    }

    async getById(userId: string, messageId: string): Promise<MessageWithSender | null> {
