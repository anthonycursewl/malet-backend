...
export class ManageKeysService implements ManageKeysUseCase {
  ...
  async registerKey(
    ...
    const existingKey = await this.keyRepository.findByUserAndDevice(
      userId,
      deviceId,
    );
    ...
    const keyByFingerprint =
      await this.keyRepository.findByFingerprint(keyFingerprint);
    if (keyByFingerprint && keyByFingerprint.getUserId() !== userId) {
      throw new BadRequestException(
        'Esta clave ya está registrada por otro usuario',
      );
    }

    // Crear nueva clave
    const newKey = UserPublicKey.create(
      userId,
      deviceId,
      publicKey,
      keyFingerprint,
