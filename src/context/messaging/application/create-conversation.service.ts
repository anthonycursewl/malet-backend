...
export class CreateConversationService implements CreateConversationUseCase {
    ...
    async createPrivate(userId: string, params: CreatePrivateConversationParams): Promise<CreateConversationResult> {
        ...
        const existingConversation = await this.conversationRepository.findPrivateBetweenUsers(
            userId,
            participantUserId
        );
        ...
        // Crear participantes
        const participant1 = ConversationParticipant.createAsAdmin(savedConversation.getId(), userId);
        const participant2 = ConversationParticipant.create(savedConversation.getId(), participantUserId);

        const participants = await this.participantRepository.saveMany([participant1, participant2]);
    ...
    async createForCommunity(userId: string, params: CreateCommunityConversationParams): Promise<CreateConversationResult> {
        ...
        if (community.owner_id !== userId) {
            throw new ForbiddenException('Solo el owner puede crear el chat de la comunidad');
        }

        // Verificar si ya existe un chat para esta comunidad
        const existingConversation = await this.conversationRepository.findByCommunityId(communityId);
        ...
        const ownerParticipant = ConversationParticipant.createAsAdmin(savedConversation.getId(), userId);
        ...
        const participants = await this.participantRepository.saveMany(allParticipants);
