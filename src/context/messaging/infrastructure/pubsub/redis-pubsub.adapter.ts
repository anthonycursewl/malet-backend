 ...
 */
@Injectable()
export class RedisPubSubAdapter implements PubSubPort, OnModuleInit, OnModuleDestroy {
    ...
    async onModuleInit() {
        ...
        try {
            ...
            this.logger.warn('⚠️ Falling back to local-only mode (messages won\'t sync across servers)');
            return;
        }
    ...
    async publishPresence(event: PresenceEvent): Promise<void> {
        ...
        this.logger.debug(`📤 Published presence: ${event.userId} -> ${event.status}`);
    }
