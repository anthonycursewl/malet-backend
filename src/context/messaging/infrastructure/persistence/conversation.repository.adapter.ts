...
export class ConversationRepositoryAdapter implements ConversationRepository {
    ...
    async findPrivateBetweenUsers(userId1: string, userId2: string): Promise<Conversation | null> {
    ...
    async countByUserId(userId: string, type?: ConversationType): Promise<number> {
        const whereClause: any = {
            participants: {
                some: {
                    user_id: userId,
