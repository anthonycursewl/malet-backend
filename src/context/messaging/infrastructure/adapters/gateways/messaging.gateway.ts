  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleConnection(client: AuthenticatedSocket) {
    try {
      ...
      client.deviceId =
        (client.handshake.query.deviceId as string) || 'unknown';
  ...
  async handleTypingStart(
    ...
    this.logger.log(
      `📝 typing:start received from ${client.userId} for conversation ${data?.conversationId}`,
    );
    const userId = client.userId;
    if (!userId) {
  ...
  private async joinUserConversations(client: Socket, userId: string) {
    const participations = await this.participantRepository.findByUserId(
      userId,
      true,
    );
    ...
    this.logger.debug(
      `User ${userId} joined ${participations.length} conversation rooms`,
    );
  }
