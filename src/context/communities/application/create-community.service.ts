...
export class CreateCommunityService implements CreateCommunityUseCase {
    ...
    async execute(userId: string, params: CreateCommunityParams): Promise<Community> {
        // Generar slug único
        let slug = Community.generateSlug(params.name);
        let attempts = 0;
        const maxAttempts = 5;

        while (await this.communityRepository.existsBySlug(slug) && attempts < maxAttempts) {
            slug = Community.generateSlug(params.name);
            attempts++;
        }

        if (attempts >= maxAttempts) {
            throw new ConflictException('No se pudo generar un slug único para la comunidad');
        }
