...
export class ManageMembersService implements ManageMembersUseCase {
   ...
   */
  private async verifyRequesterPermissions(
    requesterId: string,
    communityId: string,
  ): Promise<CommunityMember> {
    const requesterMember = await this.memberRepository.findByUserAndCommunity(
      requesterId,
      communityId,
    );
  ...
  async approveMember(
    ...
    if (!member.isPending()) {
      throw new ForbiddenException(
        'El miembro no tiene una solicitud pendiente',
      );
    }
  ...
  async kickMember(
    ...
    const requester = await this.verifyRequesterPermissions(
      requesterId,
      communityId,
    );
    const member = await this.getMemberOrFail(memberId);
    ...
    if (member.isOwner()) {
      throw new ForbiddenException(
        'No puedes expulsar al owner de la comunidad',
      );
    }
    ...
    if (member.isAdmin() && !requester.isOwner()) {
      throw new ForbiddenException(
        'Solo el owner puede expulsar administradores',
      );
    }
  ...
  async banMember(
    ...
    if (member.isAdmin() && !requester.isOwner()) {
      throw new ForbiddenException(
        'Solo el owner puede banear administradores',
      );
    }

    const wasActive = member.isActive();

    const bannedMember = await this.memberRepository.updateStatus(
      memberId,
  ...
  async unbanMember(
    ...
    const unbannedMember = await this.memberRepository.updateStatus(
      memberId,
      MembershipStatus.ACTIVE,
    );
  ...
  async changeRole(
    ...
    const requester = await this.memberRepository.findByUserAndCommunity(
      requesterId,
      communityId,
    );
    ...
    if (!requester.isOwner()) {
      throw new ForbiddenException(
        'Solo el owner puede cambiar roles de miembros',
      );
    }
    ...
    if (newRole === MemberRole.OWNER) {
      throw new ForbiddenException(
        'Para transferir ownership usa el método específico',
      );
    }
