...
export class JoinCommunityService implements JoinCommunityUseCase {
    ...
    async execute(userId: string, communityId: string): Promise<JoinCommunityResult> {
        ...
        const existingMember = await this.memberRepository.findByUserAndCommunity(userId, communityId);
        if (existingMember) {
