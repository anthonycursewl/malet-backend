...
import { CommunityMember, CommunityMemberPrimitives } from '../../domain/entities/community-member.entity';
...
export class CommunityMemberRepositoryAdapter implements CommunityMemberRepository {
    ...
    async findByUserAndCommunity(userId: string, communityId: string): Promise<CommunityMember | null> {
    ...
    async findActiveByCommunityId(communityId: string): Promise<CommunityMember[]> {
        const members = await this.prisma.community_member.findMany({
            where: {
                community_id: communityId,
    ...
    async findPendingByCommunityId(communityId: string): Promise<CommunityMember[]> {
        const members = await this.prisma.community_member.findMany({
            where: {
                community_id: communityId,
    ...
    async updateStatus(memberId: string, status: MembershipStatus): Promise<CommunityMember> {
    ...
    async updateRole(memberId: string, role: MemberRole): Promise<CommunityMember> {
        const updated = await this.prisma.community_member.update({
            where: { id: memberId },
            data: {
                role,
