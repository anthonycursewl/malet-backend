...
import {
  ChatWithAIUseCase,
  ChatWithAIParams,
} from '../domain/ports/in/chat-with-ai.usecase';
import {
  AI_PROVIDER_REPOSITORY,
  AIProviderRepository,
} from '../domain/ports/out/ai-provider.repository';
import {
  AIChatRequest,
  AIChatResponse,
} from '../domain/entities/ai-chat.entity';
...
export class ChatWithAIService implements ChatWithAIUseCase {
  ...
  async execute(
    ...
    if (resolvedModel) {
      const isAvailable =
        await this.aiProviderRepository.isModelAvailable(resolvedModel);
      if (!isAvailable) {
