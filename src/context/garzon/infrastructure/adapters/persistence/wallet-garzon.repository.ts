...
export class SuperGarzonWalletAdapter implements WalletGarzonRepository {
    ...
    async getClientWallets(params: GetWalletParams): Promise<ClientWalletResponse> {
        try {
            console.log(`[WalletAdapter] Fetching wallets for identifier: ${params.identifier}`);
            ...
            if (apiResponse.code !== 200 || !apiResponse.data || apiResponse.data.length === 0) {
                throw new NotFoundException(`No se encontraron wallets para el cliente ${params.identifier}`);
            }
            ...
            console.error('[WalletAdapter] Error fetching wallets:', error?.message || error);

            if (error?.response?.status === 404) {
                throw new NotFoundException(`Cliente ${params.identifier} no encontrado`);
            }
            ...
            throw new InternalServerErrorException('Error al consultar las wallets del cliente');
        }
    }
    ...
    async generateToken(params: GenerateWalletTokenParams): Promise<WalletTokenResponse> {
        try {
            console.log(`[WalletAdapter] Generating token for ${params.wallets.length} wallet(s)`);
            ...
            return {
                ...
                message: apiResponse.message || (apiResponse.code === 200 ? 'Token generado exitosamente' : 'Error al generar token'),
                data: apiResponse.data,
                requestedWallets: params.wallets.length,
            };

        } catch (error: any) {
            console.error('[WalletAdapter] Error generating token:', error?.message || error);

            throw new InternalServerErrorException('Error al generar token para las wallets');
        }
    }
