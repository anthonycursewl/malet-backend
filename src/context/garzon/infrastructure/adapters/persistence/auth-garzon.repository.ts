...
import {
  AuthCredentials,
  AuthSession,
} from 'src/context/garzon/domain/entities/auth.entity';
    ...
    try {
      ...
      if (!csrfTokenHtml) {
        throw new InternalServerErrorException(
          'No se pudo extraer el CSRF token del HTML',
        );
      }
      ...
      if (!rawXsrfCookie)
        throw new InternalServerErrorException(
          'Cookie XSRF-TOKEN no encontrada',
        );
      ...
      if (loginResponse.status === 401 || loginResponse.status === 422) {
        throw new UnauthorizedException(
          'Credenciales inválidas en sistema legado',
        );
      }
      return {
