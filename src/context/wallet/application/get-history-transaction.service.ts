...
export class GetHistoryTransactionService implements GetHistoryTransactionUseCase {
    ...
    async execute(id: string, skip: number, take: number, type: string, user_id: string): Promise<Transaction[]> {
        return this.transactionRepository.getHistoryTransaction(id, skip, take, type, user_id)
    }
}