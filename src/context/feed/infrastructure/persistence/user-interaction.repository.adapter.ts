...
import { UserInteraction, InteractionType } from '../../domain/entities/user-interaction.entity';
...
export class UserInteractionRepositoryAdapter implements UserInteractionRepository {
    ...
    async findByUserId(userId: string, limit: number = 100): Promise<UserInteraction[]> {
    ...
    async findByUserAndCommunity(userId: string, communityId: string): Promise<UserInteraction[]> {
        const interactions = await this.prisma.user_interaction.findMany({
            where: {
                user_id: userId,
    ...
    async countByCommunityAndType(communityId: string, interaction: InteractionType): Promise<number> {
