...
import { CommunityScore, CommunityScorePrimitives } from '../domain/entities/community-score.entity';
import {
...
export class RecommendationEngineService {
     ...
     */
    async calculateScore(userId: string, community: CommunityData): Promise<CommunityScore> {
    ...
    private async calculateFreshness(lastActivity: Date): Promise<number> {
        ...
        // Decay exponencial
        return 100 * Math.exp(-hoursSinceActivity / this.config.freshnessDecayHours);
    }
     ...
     */
    async getCommunityDataForCalculation(communityIds: string[]): Promise<CommunityData[]> {
