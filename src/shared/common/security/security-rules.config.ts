...
export const DANGEROUS_EXTENSIONS = /\.(php|env|asp|aspx|jsp|cgi|bak|sql|htaccess|htpasswd|exe|dll)$/i;

// Path traversal attempts 
export const PATH_TRAVERSAL = /(\.\.[\/\\]|%2e%2e%2f|%2e%2e\/|\.\.%2f|%2e%2e%5c)/i;

// SQL Injection patterns 
export const SQL_INJECTION_URL = /(union\s+(all\s+)?select|'\s*or\s*'1'\s*=\s*'1|'\s*or\s*1\s*=\s*1|admin'\s*--|'\s*;\s*drop\s|'\s*;\s*delete\s|'\s*;\s*insert\s|'\s*;\s*update\s)/i;

// XSS patterns 
export const XSS_PATTERNS = /(<script[\s>]|javascript\s*:|<iframe[\s>]|<object[\s>]|<embed[\s>]|onerror\s*=|onload\s*=)/i;

// Command injection pattern
export const COMMAND_INJECTION = /(\$\(|`[^`]+`|;\s*(cat|ls|rm|wget|curl|bash|sh|nc|netcat)\s|%0a|%0d)/i;
