...
export class TransactionPolicy implements Policy<Transaction> {
     ...
     */
    canRead(user: AuthenticatedUser, transaction: Transaction & { account?: Account }): boolean {
     ...
     */
    canUpdate(user: AuthenticatedUser, transaction: Transaction): boolean {
     ...
     */
    canDelete(user: AuthenticatedUser, transaction: Transaction & { account?: Account }): boolean {
        if (!transaction.account) {
            return false;
        }
        return transaction.account.getUserId() === user.userId;
