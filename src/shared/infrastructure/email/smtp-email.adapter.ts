...
export class SmtpEmailAdapter implements EmailServicePort {
    ...
    private initializeTransporter(): void {
        ...
        if (!host || !user || !pass) {
            ...
            this.logger.warn('Required: HOST_EMAIL, PORT_EMAIL, SMTP_USER, SMTP_PASSWORD');
        }
    ...
    async sendEmail(params: SendEmailParams): Promise<EmailResult> {
        try {
            ...
            const from = params.from
                ? (params.fromName ? `"${params.fromName}" <${params.from}>` : params.from)
                : this.fromAddress;
    ...
    async sendVerificationEmail(params: VerificationEmailParams): Promise<EmailResult> {
        const template = EmailTemplates.verification(params.username, params.verificationCode);
    ...
    async sendPasswordResetEmail(params: PasswordResetEmailParams): Promise<EmailResult> {
        const template = EmailTemplates.passwordReset(params.username, params.resetCode);
